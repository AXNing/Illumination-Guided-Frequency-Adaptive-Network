...
# Resize flare image to match base image size
		  # Get (height, width) of base_img
		base_size = base_img.size 
		flare_img = flare_img.resize(base_size, Image.BILINEAR)
		base_size = base_img.size 
		...
		if self.reflective_flag:
				...
				reflective_img = reflective_img.resize(base_size, Image.BILINEAR)
		...
		if self.mask_type==None:
			...
			flarepath = flare_path.split("/")[-1]
			gtpath = img_path.split("/")[-1]

			return adjust_gamma_reverse(base_img),adjust_gamma_reverse(flare_img),adjust_gamma_reverse(merge_img),flare_mask,imgpath,flarepath,gtpath
