...
from basicsr.pytorch_wavelets import DWTForward, DWTInverse
import basicsr.dataset_utils
import torchvision
...
def train_pipeline(root_path):
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        while train_data is not None:
            ...
            target = train_data['gt'].cuda()
            input_ = train_data['lq'].cuda()


            target, input_aug, mask, aug = basicsr.dataset_utils.apply_augment(target, input_, xfm1, ifm1, augs=["Swap_Fre", "none"], mix_p=[0.3,0.7], component=-1)
            if aug == "Swap_Fre":
                    input_ = 0.4 * input_aug + (1 - 0.4) * input_
            else:
                    input_ = input_aug

            train_data['lq'].copy_(input_)
