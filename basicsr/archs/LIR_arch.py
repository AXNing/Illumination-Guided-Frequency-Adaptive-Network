...
from basicsr.archs.filters import *
import torch
...
class Filters(nn.Module):
    ...
    def forward(self, x):
        if self.weight is None:
            ...
            self.Sobel_xy = self.Sobel_xy.to(device)
            self.Sobel_yx = self.Sobel_yx.to(device)
            self.Roberts_x = self.Roberts_x.to(device)
            self.Roberts_y = self.Roberts_y.to(device)
            self.Laplation = self.Laplation.to(device)
...
class LIR(nn.Module):
    def __init__(self, img_channel=3, img_size=512,output_ch=3,dim=48, left_blk_num=[3, 3], bottom_blk_num=3, right_blk_num=[4, 3], training=True):
        super().__init__()
        self.training = training
