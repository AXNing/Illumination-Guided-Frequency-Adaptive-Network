...
import pywt
import numpy as np
from basicsr.archs.wavelet import IWT,DWT
...
class DynamicFilter(nn.Module):
    def __init__(self, dim, expansion_ratio=2, reweight_expansion_ratio=.25,
                 ...
                 bias=False, num_filters=4, size=14, weight_resize=False,wavelet='haar',
                 **kwargs):
        super().__init__()
        size = to_2tuple(size)
        self.size = size[0] // 2
        self.filter_size = size[1] // 2 
        ...
        self.pwconv1 = nn.Linear(dim, self.med_channels, bias=bias)
        self.act1 = act1_layer()
        self.reweight_cA = Mlp(dim, reweight_expansion_ratio, num_filters * self.med_channels)
        self.reweight_cHVD = Mlp(dim, reweight_expansion_ratio, num_filters * self.med_channels)


        self.complex_weights1 = nn.Parameter(
            torch.randn(self.size, self.filter_size, num_filters, 
                        dtype=torch.float32) * 0.02)
        # self.complex_weights2 = nn.Parameter(
        #     torch.randn(self.size, self.filter_size, num_filters, 
        #                 dtype=torch.float32) * 0.02)
        # self.complex_weights3 = nn.Parameter(
        #     torch.randn(self.size, self.filter_size, num_filters, 
        #                 dtype=torch.float32) * 0.02)
        # self.complex_weights4 = nn.Parameter(
        #     torch.randn(self.size, self.filter_size, num_filters, 
        #                 dtype=torch.float32) * 0.02)
        self.act2 = act2_layer()
        self.pwconv2 = nn.Linear(self.med_channels, dim, bias=bias)
    ...
    def forward(self, x):
        ...
        # a = x.permute(0, 3, 1 ,2)
        # a = x[:, :3, :, :]
        # torchvision.utils.save_image(a[0],"/home/ubuntu/axproject/Flare7K/vis/wav_x_in.png")
        #x = x.permute(0, 2,3 ,1)

        # routeing = self.reweight(x.mean(dim=(1, 2))).view(B, self.num_filters,
        #                                                   -1).softmax(dim=1)#计算动态权重系数，x.mean(dim=(1, 2)))，dim=(1,2)表示对高和宽进行平均池化，x。mean计算每个样本中所有像素点的平均值，只保留每个通道的全局特征。变成一个每个通道的平均值的张量，形状是 (B, C)，其中每一行包含一个图像的每个通道的均值。
        
        #self.reweight是mlp，接受x.mean(dim=(1, 2)))，将其映射到num_filters * med_channels的输出空间，输出的形状为(B, num_filters * med_channels)，每个样本有num_filters通道，每个通道有med_channels输出
        #然后reshape为(B, self.num_filters, med_channels)，再归一化
        #减少空间维度的复杂性，为后续的动态加权提供了一个简化的表示，每个通道的特征均值作为输入，帮助模型生成针对每个图像特征的合适滤波器加权系数。
        #基于输入图像 x 的 平均值（在空间维度上，即在高和宽上求均值）来生成一个用于加权的系数

        # 计算cA权重
        routeing_cA = self.reweight_cA(x.mean(dim=(1, 2))).view(B, self.num_filters, -1).softmax(dim=1)
        
        # 计算cH、cV、cD的权重
        routeing_cH = self.reweight_cHVD(x.mean(dim=(1, 2))).view(B, self.num_filters, -1).softmax(dim=1)
        routeing_cV = self.reweight_cHVD(x.mean(dim=(1, 2))).view(B, self.num_filters, -1).softmax(dim=1)
        routeing_cD = self.reweight_cHVD(x.mean(dim=(1, 2))).view(B, self.num_filters, -1).softmax(dim=1)

        x = self.pwconv1(x)

        x = self.act1(x)#(b,h,w,c)
        #x = x.permute(0, 3, 1 ,2)#(b,c,h,w)

        #x = x.to(torch.float32)
        #x = torch.fft.rfft2(x, dim=(1, 2), norm='ortho')
        #x_cpu = x.detach().cpu()
        #coeffs = pywt.dwt2(x_cpu, self.wavelet, mode='periodization')
        #cA, (cH, cV, cD) = coeffs  # 低频和高频分量

        cA, cH, cV, cD = self.wavelet(x)#(b,h,w,c)


        # cH = cH.permute(0, 2,3,1)
        # cV = cV.permute(0, 2,3,1)
        # cD = cD.permute(0, 2,3,1)
        # cA = cA.permute(0, 2,3,1)  
        # cA = np.transpose(cA, (0, 2, 3, 1))      
        # cH = np.transpose(cH, (0, 2, 3, 1))      
        # cV = np.transpose(cV, (0, 2, 3, 1))      
        # cD = np.transpose(cD, (0, 2, 3, 1))      


        if self.weight_resize:
            complex_weights1 = resize_complex_weight(self.complex_weights1, cA.shape[1],
                                                    cA.shape[2])#(h,w)
            #complex_weights1 = torch.view_as_complex(complex_weights1.contiguous())
            # complex_weights2 = resize_complex_weight(self.complex_weights2, cH.shape[1],
            #                                         cH.shape[2])
            # #complex_weights2 = torch.view_as_complex(complex_weights2.contiguous())
            # complex_weights3 = resize_complex_weight(self.complex_weights3, cV.shape[1],
            #                                         cV.shape[2])
            # complex_weights4 = resize_complex_weight(self.complex_weights4, cD.shape[1],
            #                                         cD.shape[2])
        else:
            complex_weights1 = self.complex_weights1
            # complex_weights2 = self.complex_weights2
            # complex_weights3 = self.complex_weights3
            # complex_weights4 = self.complex_weights4
        # routeing_cA = routeing_cA.to(torch.complex64)
        # routeing_cHVD = routeing_cHVD.to(torch.complex64)
        cA_weight = torch.einsum('bfc,hwf->bhwc', routeing_cA, complex_weights1)
        # cH_weight = torch.einsum('bfc,hwf->bhwc', routeing_cH, complex_weights2)
        # cV_weight = torch.einsum('bfc,hwf->bhwc', routeing_cV, complex_weights3)
        # cD_weight = torch.einsum('bfc,hwf->bhwc', routeing_cD, complex_weights4)
...
#        x = x * weight
        ...
        # cA_weight = cA_weight.permute(0, 3, 1 ,2)
        # cHVD_weight = cHVD_weight.permute(0, 3, 1 ,2)

        # cA = torch.from_numpy(cA).to(cA_weight.device) * cA_weight
        # cH = torch.from_numpy(cH).to(cHVD_weight.device) * cHVD_weight
        # cV = torch.from_numpy(cV).to(cHVD_weight.device) * cHVD_weight
        # cD = torch.from_numpy(cD).to(cHVD_weight.device) * cHVD_weight
        ...
        # cD = cD.permute(0, 3,1,2)
        # cA = cA.permute(0, 3,1,2)

        # coeffs_filtered = (cA, (cH, cV, cD))
        # x = torch.tensor(pywt.idwt2(coeffs_filtered, self.wavelet, mode='periodization')).to(x.device)

        x = self.iwavelet(torch.cat((cA,cH,cV,cD),0))
        #x = x.permute(0, 2, 3 ,1)
        x = self.act2(x)
        x = self.pwconv2(x)
        #x = x.permute(0, 3, 1 ,2)
        # a = x.permute(0, 3, 1 ,2)
        # a = a[:, :3, :, :]
        # torchvision.utils.save_image(a[0],"/home/ubuntu/axproject/Flare7K/vis/fft_x_out.png")
        #x = x.permute(0, 2,3 ,1)
