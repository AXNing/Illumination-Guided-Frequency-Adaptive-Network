...
from basicsr.archs.CrossAttention import CrissCrossAttention
...
class RepRFB(nn.Module):
    def __init__(self, feature_nums, act_type='lrelu', deploy=False):
        ...
        #self.crisscross = CrissCrossAttention(in_dim=feature_nums)
    ...
    def forward(self, inputs):
        ...
        #outputs = self.crisscross(outputs)
...
class RepRFN(nn.Module):
    def __init__(self, in_channels=3, output_ch=6, feature_nums=48, upscale_factor=4,
                 deploy=True):
        ...
        self.lr_conv1 = conv_layer(in_channels=feature_nums, out_channels=out_channels, kernel_size=3)
        self.act = nn.Sequential(nn.Sigmoid())
        self.criss = CrissCrossAttention(in_dim=48)
    def forward(self, inputs):
        ...
        #print(outputs.size())
        outputs = self.criss(outputs)
        outputs = outputs + outputs_feature
        outputs = self.lr_conv1(outputs)
