...
import torchvision
from basicsr.archs.k_an import KAN 
def _cfg(url='', **kwargs):
    return {
        'url': url,
...
class DynamicFilter(nn.Module):
    def __init__(self, dim, expansion_ratio=2, reweight_expansion_ratio=.25,
        ...
        hidden = int(reweight_expansion_ratio*dim)
        outdim = int(num_filters * self.med_channels)
        self.kan = KAN([dim,hidden,outdim])
        self.complex_weights = nn.Parameter(
    ...
    def forward(self, x,retinex):
        B, H, W, _ = x.shape

        routeing = self.kan(retinex.mean(dim=(1, 2))).view(B, self.num_filters,
                                                          -1).softmax(dim=1)
